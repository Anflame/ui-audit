...
export const collectImportsSet = (
  astFile: t2.File,
): { antdLocals: Set<string>; allLocals: Set<string>; moduleByLocal: Map<string, string> } => {
    ...
    ImportDeclaration: (p) => {
      ...
      for (const sp of p.node.specifiers) {
        if (t2.isImportSpecifier(sp) || t2.isImportDefaultSpecifier(sp) || t2.isImportNamespaceSpecifier(sp)) {
          ...
          moduleByLocal.set(sp.local.name, src);
  ...
  return { antdLocals, allLocals, moduleByLocal };
};
